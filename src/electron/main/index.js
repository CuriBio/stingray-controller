...
const start_python_subprocess = () => {
  ...
  if (process.argv.includes("--log-level-debug"))
    python_cmd_line_args.push("--log-level-debug");
  if (process.platform !== "win32") {
    // presumably running in a unix dev or CI environment
    if (!ci.isCI) {
      // don't do this in CI environment, only locally
      python_cmd_line_args.push("--skip-software-version-verification");
    }
  }
  ...
  if (isRunningInBundle()) {
    ...
    const python_subprocess = require("child_process").execFile(
      script,
      python_cmd_line_args
    );
    ...
    if (!store.get("beta_2_mode"))
      python_cmd_line_args.push("--no-load-firmware");
...
const CLOUD_DOMAIN =
  CLOUD_ENDPOINT_VALID_OPTIONS[CLOUD_ENDPOINT_USER_OPTION] || "curibio-test";
...
const post_latest_software_version = (version) => {
  ...
  const post_interval_id = setInterval(() => {
    if (!awaiting_response) {
      ...
      axios
        .post(
          `http://localhost:${flask_port}/latest_software_version?version=${version}`
        )
        .then((response) => {
          console.log(
            `/latest_software_version response: ${response.status} ${response.statusText}`
          ); // allow-log;
...
const set_up_auto_updater = () => {
  ...
  autoUpdater.checkForUpdates().catch((response) => {
    console.log(
      "Error while checking for updates: " + JSON.stringify(response)
    ); // allow-log
    const current_version = get_current_app_version();
    post_latest_software_version(current_version);
  });
...
app.on("second-instance", (event, command_line, working_directory) => {
  ...
  if (win_handler && win_handler.browserWindow) {
    if (win_handler.browserWindow.isMinimized())
      win_handler.browserWindow.restore();
...
app.once("will-quit", function (e) {
  ...
  axios
    ...
    .then((response) => {
      console.log(
        `Flask shutdown response: ${response.status} ${response.statusText}`
      ); // allow-log
      quit_app();
    })
    .catch((response) => {
...
const quit_app = () => {
  const auto_install_str = autoUpdater.autoInstallOnAppQuit
    ? "enabled"
    : "disabled";
  console.log(
    // allow-log
...
const exit_app_clean = () => {
  ...
  const wait_for_subprocess_to_complete_with_timeout = new Promise(
    (resolve) => {
